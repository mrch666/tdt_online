...
### Веб-интерфейс для управления внешними изображениями
- **Главная страница**: `GET /web/external-images/` - просмотр всех товаров с внешними изображениями
- **Обработка изображения**: `POST /web/external-images/{image_id}/process` - скачивание, конвертация и загрузка изображения в основную БД

#### Особенности веб-интерфейса:
- **Группировка по товарам**: изображения сгруппированы по товарам для удобства навигации
- **Миниатюры 150x150px**: предпросмотр изображений в сетке миниатюр
- **Статусы изображений**: визуальная индикация статусов (необработанное, успешно загружено, ошибка загрузки)
- **Фильтрация**: возможность скрыть товары с успешно загруженными изображениями
- **Однокликовая обработка**: обработка изображения одним кликом (скачивание → конвертация в JPG → загрузка в БД)
- **AJAX обработка**: динамическая обработка без перезагрузки страницы
- **Уведомления**: система уведомлений о результатах обработки
...
Откройте в браузере: `http://localhost:7990/web/external-images/`
