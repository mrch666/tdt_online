...
4. **Обновляет запись в БД**: устанавливает поле `imgext` в `'jpg'` и `changedate` в текущую дату
5. Проверяет, что файл успешно сохранен на диск
6. Проверяет обновленную информацию о изображении в базе данных (поле `imgext` и `changedate`)
7. Удаляет тестовый файл через хранимую процедуру `wp_DeleteFile`
