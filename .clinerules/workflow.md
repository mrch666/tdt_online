...
5. Создание теста, который проходит после исправления
   ↓
6. Автоматический запуск всех предыдущих тестов
   ↓
7. Коммит только после прохождения всех тестов
```

### 4.1 Автоматический запуск тестов
**Перед любым изменением кода необходимо:**
1. **Запустить все существующие тесты** командой: `python -m pytest tests/ -v`
2. **Убедиться, что все тесты проходят** перед внесением изменений
3. **Если тесты не проходят** - сначала исправить существующие проблемы

**После внесения изменений необходимо:**
1. **Запустить все тесты снова** для проверки регрессии
2. **Убедиться, что новые изменения не сломали существующий функционал**
3. **Запустить тесты для измененного файла** отдельно: `python -m pytest tests/ -v -k "test_name"`

**Автоматизация:**
- Всегда использовать `python -m pytest tests/` для запуска всех тестов
- Использовать `-v` флаг для детального вывода
- Использовать `--tb=short` для краткого вывода ошибок
- Логировать результаты тестов в файл при необходимости
