...
def get_models_by_id(searchtext):
    return jsonify(json_list=[ (s.to_dict(),m.to_dict(),v.to_dict(),vl.to_dict(),fl.to_dict()) for s,m,v,vl,fl in db.session.query(Storage,Modelgood, Vollink,Vol,Folder).\
        ...
        filter(Storage.id != '0rfarg000os1'). \
        filter(Storage.id != '0rfarg000FZh'). \
