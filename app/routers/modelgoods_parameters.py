...
from app.models import ModelGoodsParameter
        ...
        if product_id and params:
            ...
            try:
                db.execute(
                    ...
                    {
                        'modelid': product_id,
                        "xml_content": params_xml.encode('windows-1251')
                    }
                ).fetchall()
                logger.info(f"Saved bytes sample (WINDOWS-1251): {params_xml.encode('windows-1251')[:100].hex()}")
                db.commit()
...
async def get_parameters(
    ...
    PAGE_SIZE = 10
    base_query = db.query(ModelGoodsParameter)
    
    if search:
        search_filter = or_(
            ModelGoodsParameter.name.ilike(f"%{search}%"),
            ModelGoodsParameter.value.ilike(f"%{search}%")
        )
